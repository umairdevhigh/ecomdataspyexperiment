...
# PAGE CONFIG
# ============================================================
st.set_page_config(page_title="Universal E-commerce Extractor V5.5", page_icon="🛒")
st.title("🛒 UNIVERSAL E-COMMERCE EXTRACTOR V5.5 (AI PERFECT)")
st.markdown("**Gemini AI with Custom Prompt | Variable Products | SEO-Optimized Content**")
...
# ============================================================
# CONTENT SETTINGS (WITH CUSTOM PROMPT)
# ============================================================
...
with st.expander("⚙️ Configure Product Content", expanded=True):
    ...
    if st.session_state.get("ai_enabled", False):
        ...
        with col_key2:
            st.selectbox("Model", ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-2.0-flash-exp"], key="gemini_model",
                         help="Flash = fastest, Pro = best quality, Exp = experimental.")
        
        # 🔥 NEW: Custom Prompt for AI Writer
        st.text_area("✍️ Custom Prompt (Train the AI writer)", key="ai_custom_prompt",
                     placeholder="e.g. Write a luxurious, persuasive product description for premium leather jackets, focusing on quality and craftsmanship. Use a tone that's elegant and aspirational.",
                     height=120,
                     help="Is prompt ko Gemini ko bheja jayega. Aap apni brand voice, target audience, aur tone define kar sakte hain.")
...
class SmartRewriter:
    ...
    def generate_seo_content(self, title, raw_desc, category=None, store_context=None, specs_text=None):
        ...
        feature_html = ''.join(f"<li>{item}</li>" for item in all_bullets[:8]) if all_bullets else ""
...
# ============================================================
# GEMINI AI DESCRIPTION GENERATOR (WITH CUSTOM PROMPT)
# ============================================================
def generate_ai_description_gemini(title, specs, category, store_context, api_key, model_name, custom_prompt):
    if not api_key or not title:
        return None
    
    # Build prompt with custom instructions
    base_prompt = f"""You are an expert e-commerce copywriter for a store: {store_context if store_context else 'Premium Products'}.
...
"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Log error but return None so fallback works
            return None
    except Exception:
        return None
...
def parse_ai_response(text, fallback_title):
    ...
    if not result['short_description']:
        result['short_description'] = f"Shop the {fallback_title} today."
    if not result['seo_title']:
        result['seo_title'] = f"{fallback_title} — Premium Quality"
...
# ============================================================
# SMART TITLE GENERATOR (IMPROVED)
# ============================================================
def generate_smart_title(original_title, specs_text, color=None, material=None):
    ...
    if detected_material:
        parts.append(detected_material)
    parts.append(core)
    if detected_color and detected_color not in ' '.join(parts):
...
# ============================================================
# 🔥 FIXED: VARIABLE PRODUCTS EXTRACTION (Improved)
# ============================================================
def extract_variations_from_jsonld(product_data, soup, base_url_domain, price):
    """Extract variations from multiple JSON-LD contexts and HTML fallback"""
    variations = []
    seen_attrs = set()
    ...
    # Method 4: HTML fallback (size select + color swatches)
    if not variations:
        size_options = []
        color_options = []
...
# ============================================================
# MAIN SCRAPER (FIXED: VARIABLE PRODUCTS + AI + CUSTOM PROMPT)
# ============================================================
def scrape_product(url, session, config):
    ...
    # Ensure non-empty
    if not long_desc or len(long_desc.strip()) < 10:
        long_desc = f"<p>Discover the {title}. Crafted with premium materials for lasting quality and style. Order now.</p>"
    if not gen_short_desc or len(gen_short_desc.strip()) < 5:
        gen_short_desc = f"Shop the {title} today."
    if not seo_title or len(seo_title.strip()) < 5:
        seo_title = f"{title} — Premium Quality"
    if not gen_seo_description or len(gen_seo_description.strip()) < 5:
        gen_seo_description = f"Discover the {title} collection today."
    ...
    # ============================================================
    # 🔥 EXTRACT VARIATIONS (IMPROVED)
    # ============================================================
    variations_data = extract_variations_from_jsonld(product_data, soup, base_url_domain, price)
    
    # If still no variations, try to see if product is actually variable
    # but no data found; treat as simple.
...
st.caption("🛒 V5.5 FINAL | Gemini AI with Custom Prompt | Variable Products | SEO-Optimized | Batch Mode")